...
四、板块关联
   概念热度（8日序列+阶段）已传给你。**对齐时间轴**：个股启动段、回踩段
   分别落在板块序列的哪几天，然后归入以下之一：
   - 回踩 + 板块升温/持续 → 同步，板块在托
   - 回踩 + 板块退潮 → 背离，个股随板块一起走弱
   - 启动段恰好对应板块的单日脉冲（一日游）→ ⚠️ 这次启动可能只是蹭脉冲，性质可疑
   都不沾就说「与板块无明显关联」。板块序列只有 8 天，不要说成「长期趋势」。
   板块关联**没有实测基准**，只作定性提示，不得据此改动第二段的档位。

【硬性要求】
- 用短标题分四段写，每段 2~3 句，全文 400 字以内
...
请按「一、强支撑 二、第二波概率 三、量价异常 四、板块关联」四段作答。"""
